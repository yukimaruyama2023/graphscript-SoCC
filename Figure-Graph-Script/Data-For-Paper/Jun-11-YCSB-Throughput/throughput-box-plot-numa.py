...
# 対象ディレクトリ
log_dir = "./NUMA-LOG"  # ← ハイフンに注意

# 入力（kernel または user）
mode = input("kernel か user を入力してください: ").strip().lower()
...
# DataFrame に変換
df = pd.DataFrame({label: pd.Series(values) for label, values in data.items()})

# 可視化
plt.figure(figsize=(12, 6))
df.boxplot(column=labels)
plt.ylabel("Throughput (ops/sec)")
plt.title(f"Throughput Comparison ({mode} mode)")
plt.grid(True)
